...
w_beam = 5e-5;          % beam width parameter [rad]
...
sigma_pj = mean([std(xr), std(yr)]);  % average over x and y
...
% ------------------------------------------------------------------------
%  COMPUTE COMPLEMENTARY SENSITIVITY BANDWIDTH
% -------------------------------------------------------------------------

% Complementary sensitivity bandwidth:
% frequency where |T(jw)| = 1 (0 dB)
idx_bw = find(T_mag(1:end-1) >= 0 & T_mag(2:end) <= 0, 1, 'first');

if ~isempty(idx_bw)
    % Log-frequency interpolation for a smoother estimate
    f1 = f_vec(idx_bw);
    f2 = f_vec(idx_bw+1);
    m1 = T_mag(idx_bw);
    m2 = T_mag(idx_bw+1);

    logf_bw = interp1([m1 m2], log10([f1 f2]), 0);
    fT_bw = 10^logf_bw;
else
    fT_bw = NaN;
end

fprintf('Complementary sensitivity bandwidth: %.2f Hz\n', fT_bw);

figure('Name','S(s) and T(s) with T-bandwidth');
subplot(2,1,1);
semilogx(f_vec, S_mag, 'b', 'LineWidth', 1.5); hold on;
semilogx(f_vec, T_mag, 'r', 'LineWidth', 1.5);
yline(0, 'k--', '0 dB', 'LabelHorizontalAlignment','left');
...
if ~isnan(fT_bw)
    xline(fT_bw, 'r--', sprintf('f_{bw,T} = %.1f Hz', fT_bw), ...
        'LineWidth', 1.2, 'LabelVerticalAlignment','bottom');
end
...
ylabel('Magnitude (dB)');
title('Sensitivity S(s) and Complementary Sensitivity T(s)');
legend('S = 1/(1+L)', 'T = L/(1+L)', '0 dB', ...
    sprintf('T bandwidth = %.1f Hz', fT_bw), ...
    'Location','east');
...
% -----------------------------------------------------------------------
% PLOT G: Beta distribution vs empirical p(I)
% -----------------------------------------------------------------------

% Theoretical PDF
w = w_beam;
beta_param = w^2 / (4 * sigma_pj^2);
I_axis = linspace(0, 1, 200);
p_theory = beta_param * I_axis.^(beta_param - 1);
...
hold on;
plot(I_axis, p_theory, 'r-', 'LineWidth', 2);
xlabel('Normalized intensity I');
ylabel('PDF');
legend('Empirical (simulation)', 'Beta distribution (theory)');
title(sprintf('\\beta = %.2f', beta_param));




% -----------------------------------------------------------------------
% PLOT H: Outage probability
% -----------------------------------------------------------------------
I_thresh = 0.5;  % example threshold
%fraction of elements that satisfy I_closed < I_thresh:
P_outage_closed = mean(I_closed < I_thresh);  
P_outage_open   = mean(I_open   < I_thresh);

fprintf('Outage open-loop:   %.4f\n', P_outage_open);
fprintf('Outage closed-loop: %.4f\n', P_outage_closed);
